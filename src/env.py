...
#可得到的最大信道速度,香农定理
def trans_rate(b, p, user_loc, edge_loc):
    #B = 2e6
    B = b
    #P = 0.25
    P = p
    #print("看看这里的距离是什么形式的？", user_loc, edge_loc)
    d = np.sqrt(np.sum(np.square(user_loc[0] - edge_loc))) + 0.01
    h = 4.11 * math.pow(3e8 / (4 * math.pi * 915e6 * d), 2)
    N = 1e-10
    return B * math.log2(1 + P * h / N)

def randombin(action):
    userlist = list(bin(action).replace('0b', '')) #10进制转换成二进制，用空格替换0b
    zeros = USER_NUM - len(userlist)
    ll = [0 for i in range(zeros)] #创建全零列表（数组）
    for i in userlist:
        ll.append(int(i))
        #print("ll", ll)
    return ll
def offloadOL():
    ACTION = random.randint(1, 2 ** USER_NUM - 1)  # 函数返回参数1和参数2之间的任意整数
    # print("self.ACTION看看是什么", self.ACTION)  #这个应该就是k值
    userlist = randombin(ACTION)  # 动作转换这里可以进行改变，改成二进制卸载
    return userlist
def offloadno():
    userlist = np.zeros(USER_NUM)
    return userlist
def offloadedge():
    userlist = np.ones(USER_NUM)
    return userlist
...
def get_minimum():
    ...
    for data_num in range(TXT_NUM):
        ...
        file_path = "D:\\download contents\\论文\\思路论文\\Resources-Allocation-in-The-Edge-Computing-Environment-Using-Reinforcement-Learning-master/data/" + LOCATION + "/" + file_name
...
class UE():
    def __init__(self, user_id, data_num):
        ...
        # self.f_ue = 1e8 * 5.2  # UE的计算频率
        self.f_ue = random.uniform(1e8 * 3.2, 1e8 * 7.8)
        self.task_size = 300 * 300 * 30 * 4
        self.u_B = (1e-12)*3  # 用户能耗消耗系数，与用户结构相关
        self.p = random.uniform(2.5, 4.5)
        # self.sum_cost = 0#用来累加所有用户的cost，还要定义一个函数，用来每次释放
        self.mig_count = 0
    ...
    def request_update(self, b):
        ...
        if self.req.state == 5:
            ...
            if self.req.state == 0:
                ...
                self.req.u2e_size -= trans_rate(b, self.p, self.loc, self.req.edge_loc)
...
class EdgeServer():
    def __init__(self, edge_id, loc):
        ...
        self.m_B = 1e-12 * 5  # 用户能耗消耗系数，与用户结构相关
        self.task = 0  # 记录一下迁移数
        self.capability = random.uniform(1e8 * 6.3, 1e8 * 8.3)
...
class Env():
    def __init__(self):
        ...
        self.task_num = 0  # 用来标记任务卸载失败数
        self.num = 0   # 用来记录本地执行次数
        self.cost = [10000] * USER_NUM
        self.delay = [10000] * USER_NUM
        #self.a1 = offloadOL()
        self.a1 = [1] * USER_NUM
        print("####卸载动作的产生####", self.a1)
    def action_num(self):
        anum = 0
    ...
    def ddpg_step_forward(self, a, r_dim, b_dim):
        ...
        for user_id in range(self.user_num):
            ...
            prob_weights = abs(a[base:base + self.edge_num])
            print("动作有无负值", prob_weights, a[base:base + self.edge_num])
        ...
        for user in self.U:
            ...
            if self.a1[user.user_id] == 1:
                if self.B[user.user_id] != 0:
                    b = self.B[user.user_id]
                else:
                    b = 2e6
                user.request_update(b)
                if user.req.timer >= 5:
                ...
                if user.req.state == 4:
                    ...
                    user.generate_request(self.O[user.user_id])  # offload according to the priority
                    if self.R[user.user_id] != 0:
                        delay = user.task_size / self.R[user.user_id] + user.req.tasktype.req_u2e_size / trans_rate(b, user.p, user.loc, user.req.edge_loc)
                        delay += (user.mig_count*user.req.tasktype.migration_size) / trans_rate(b, user.p, user.loc, user.req.edge_loc)
                        delay = delay*100
                        energy = 100 * user.p * (user.req.tasktype.req_u2e_size / trans_rate(b, user.p, user.loc, user.req.edge_loc))
                        cost1 = b1 * delay + b2 * energy
                        if self.cost[user.user_id] > cost1:
                            self.cost[user.user_id] = cost1
                            # self.task_num += 1
                        if self.delay[user.user_id] > delay:
                            self.delay[user.user_id] = delay
                        self.E[int(user.req.edge_id)].task += 1
                    '''else:
                        self.task_num += 1'''
            else:
                # 计算本地时延；
                delay = user.task_size / user.f_ue
                delay = delay * 100
                energy = user.u_B * math.pow(float(user.f_ue), 2)*0.0001
                cost1 = b1 * delay + b2 * energy
                if self.cost[user.user_id] > cost1:
                    self.cost[user.user_id] = cost1
                if self.delay[user.user_id] > delay:
                    self.delay[user.user_id] = delay
                if user.req.user_id in self.E[int(user.req.edge_id)].user_group:
                    self.E[int(user.req.edge_id)].user_group.remove(user.req.user_id)
                self.U[user.user_id].req.state = 5
                self.O[user.user_id] = -2
        '''if delay > 3:
            self.task_num += 1'''
        # print("时延时延", delay)
        global cost_num
        cost_num += 1
        # self.text_delay(sum_cost, cost_num)
        # print("任务卸载失败数", self.task_num)
        # edge update
        for edge in self.E:
        ...
        sum1 = 0
        for user in self.U:
            sum1 += self.cost[user.user_id]
        self.rewards = float(100.0 / sum1)
        # self.rewards = self.fin_req_count - self.prev_count
        self.prev_count = self.fin_req_count
        # print("之前的请求数", self.prev_count)
    ...
    def text_delay(self, delay, user_id):
        # 记录UE花费,总的花费
        file_name = 'D:\download contents\论文\思路论文\Resources-Allocation-in-The-Edge-Computing-Environment-Using-Reinforcement-Learning-master/output-a/'+'output-delay'+str(USER_NUM)+'UE'+str(EDGE_NUM)+'EDGE.txt'
        with open(file_name, 'a') as file_obj:
            file_obj.write("\nUE-" + '{:d}'.format(user_id) + "delay:" + '{:.2f}'.format(delay))

    def text_rate(self, rate, episode):
        # 记录UE的任务执行完成数量。
        file_name = 'D:\download contents\论文\思路论文\Resources-Allocation-in-The-Edge-Computing-Environment-Using-Reinforcement-Learning-master/output-a/'+'output-rate'+str(USER_NUM)+'UE'+str(EDGE_NUM)+'EDGE.txt'
        with open(file_name, 'a') as file_obj:
            file_obj.write("\nepisode:" + '{:d}'.format(episode) + "rate:" + '{:.2f}'.format(rate))
